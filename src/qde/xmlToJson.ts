...
function parse(xmlString: string): ProjectJson {
  ...
  return elementToJson(doc.documentElement) as ProjectJson;
}
...
function getTargetGUID(element: Element): { targetGUID: string } | undefined {
  const targetGUID = element.getAttribute("targetGUID");
  return targetGUID ? { targetGUID } : undefined;
}

function getValue(element: Element): string | boolean | number | undefined {
...
function handleChildNodes(element: Element): {
...
function elementToJson(element: Element): unknown {
  ...
  // Handle attributes
  const processedAttrs = processAttributes(element);
  ...
  // Handle child nodes
  const { children, hasTextContent, textContent, childElementCount } = handleChildNodes(element);
...
function processAttributes(element: Element): Record<string, unknown> {
